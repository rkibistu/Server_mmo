		...
		for (size_t i = 0; i < _clients.size(); ++i) {
			if (_clients[i].revents & POLLRDNORM) {
				if (_clients[i].fd == _listenSocket) {
					...
					int bytes_received = recv(_clients[i].fd, buffer, sizeof(buffer), 0);
					if (bytes_received <= 0) {
...
void Server::HandleMessage(std::string message) {
	int length;
	NetworkTags tag;
	std::string content;
	NetworkMessages::ParseMessage(message, length, tag, content);

	switch (tag) {
	case JoinGameRequest:
		std::cout << "[JoinGameRequest]: " + message + "\n";
		break;
	default:
		break;
	}
